...
type Config struct {
	...
	// underlying model at runtime (e.g. agentgo.SwappableModel) take effect
	// on the next sub-agent run.
	Model        agentgo.ChatModel
	SystemPrompt string
	// SystemPromptMode is a host-interpreted hint controlling how
	// SystemPrompt composes with the host's base prompt. agentgo does
	...
	// plain string at the boundary so agentgo stays agnostic to enum
	...
	// sub-agent's runs — same contract as agentgo.WithToolGate for
	...
	// agentgo.WithThinkingLevel for top-level agents. A runtime override
	// installed via Runner.SetThinkingLevel takes precedence over this baseline.
	ThinkingLevel agentgo.ThinkingLevel
	...
	// agentgo.WithCacheLastMessage for top-level agents; see that option
	...
	StopGuardFactory func(agentName, task string) agentgo.StopGuard
}
...
type Runner struct {
	...
	thinkOverride map[string]agentgo.ThinkingLevel
	eventObserver func(meta RunMeta, ev agentgo.Event)
	runSeq        atomic.Int64
}
...
// a follow-up message.
func (t *Tool) SetNotifyFn(fn func(agentgo.AgentMessage)) {
	t.notifyFn = fn
}
...
func (t *Tool) SetCreateModel(fn func(name string) (agentgo.ChatModel, error)) {
	t.createModel = fn
}
...
func (r *Runner) SetThinkingLevel(agentName string, level agentgo.ThinkingLevel) {
	r.thinkMu.Lock()
	defer r.thinkMu.Unlock()
	if r.thinkOverride == nil {
		r.thinkOverride = make(map[string]agentgo.ThinkingLevel)
	}
...
func (r *Runner) resolveThinking(agentName string, base agentgo.ThinkingLevel) agentgo.ThinkingLevel {
	r.thinkMu.RLock()
...
func (r *Runner) SetEventObserver(fn func(meta RunMeta, ev agentgo.Event)) {
	r.eventObserver = fn
}
...
func (t *Tool) Execute(ctx context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	var modelOverride agentgo.ChatModel
	if p.Model != "" {
...
// agentgo.ErrStopGuard / ErrMaxTurns / provider sentinels for loop
// failures — see agentgo.ErrorKind for the stable taxonomy).
//
...
// prompt-cache keys. Progress reporting via agentgo.WithToolProgress on ctx
// works identically.
...
// keeps agentgo/subagent unaware of team-specific tools while still routing
// team spawn through one user-facing surface.
func (t *Tool) executeTeamSpawn(ctx context.Context, p params, modelOverride agentgo.ChatModel) (json.RawMessage, error) {
...
func (t *Tool) executeBackground(callerCtx context.Context, agentName, taskStr, description string, modelOverride agentgo.ChatModel) (json.RawMessage, error) {
	...
	go func() {
		...
		res, err := t.runner.run(bgCtx, agentName, taskStr, modelOverride, runOptions{
			...
			getSteeringMessages: func() []agentgo.AgentMessage {
				...
				messages := make([]agentgo.AgentMessage, 0, len(drained))
				for _, message := range drained {
					messages = append(messages, agentgo.UserMsg(message))
				}
				return messages
			},
			onEvent: func(ev agentgo.Event) {
				switch ev.Type {
				...
				case agentgo.EventMessageEnd:
					...
					if message.GetRole() == agentgo.RoleAssistant && message.Usage != nil {
						rt.Update(taskID, func(e *task.Entry) {
...
func (t *Tool) executeSingle(ctx context.Context, agentName, taskStr string, modelOverride agentgo.ChatModel) (json.RawMessage, error) {
...
// via {previous}.
func (t *Tool) executeChain(ctx context.Context, chain []chainStep, modelOverride agentgo.ChatModel) (json.RawMessage, error) {
	var previous string
...
func (t *Tool) executeParallel(ctx context.Context, tasks []taskItem, modelOverride agentgo.ChatModel) (json.RawMessage, error) {
...
func (r *Runner) run(ctx context.Context, agentName, taskStr string, modelOverride agentgo.ChatModel, opts runOptions) (RunResult, error) {
	cfg, ok := r.agents[agentName]
	if !ok {
		return RunResult{}, &UnknownAgentError{Agent: agentName, Available: r.sortedAgentNames()}
	}

	agentCtx := agentgo.AgentContext{
	...
	if cfg.OnMessage != nil {
		...
		loopCfg.OnMessage = func(msg agentgo.AgentMessage) { cfg.OnMessage(name, ts, msg) }
	}
	...
	var observe func(agentgo.Event)
	if r.eventObserver != nil {
		...
		observe = func(ev agentgo.Event) { r.eventObserver(meta, ev) }
	}

	events := agentgo.AgentLoop(ctx, []agentgo.AgentMessage{agentgo.UserMsg(taskStr)}, agentCtx, loopCfg)
	...
	for ev := range events {
		...
		case agentgo.EventToolExecStart:
			...
			if opts.reportProgress {
				agentgo.ReportToolProgress(ctx, agentgo.ProgressPayload{
					Kind:    agentgo.ProgressToolStart,
					Agent:   agentName,
					Tool:    ev.Tool,
					Summary: ev.Tool,
					Args:    ev.Args,
		...
		case agentgo.EventMessageUpdate:
			if opts.reportProgress {
				if ev.DeltaKind == agentgo.DeltaThinking {
					...
					payload := agentgo.ProgressPayload{
						Kind:      agentgo.ProgressToolDelta,
						Agent:     agentName,
						Delta:     ev.Delta,
						DeltaKind: ev.DeltaKind,
					}
					if ev.DeltaKind == agentgo.DeltaToolCall {
						if m, ok := ev.Message.(agentgo.Message); ok {
					...
					agentgo.ReportToolProgress(ctx, payload)
				}
			}
		...
		case agentgo.EventMessageEnd:
			...
			if ev.Message.GetRole() == agentgo.RoleAssistant {
				lastAssistantContent = ev.Message.TextContent()
				su.Turns++
				if opts.reportProgress {
					agentgo.ReportToolProgress(ctx, agentgo.ProgressPayload{
						Kind:    agentgo.ProgressTurnCounter,
						Agent:   agentName,
						Turn:    su.Turns,
						Summary: fmt.Sprintf("turn %d", su.Turns),
					})
				}
				if msg, ok := ev.Message.(agentgo.Message); ok && msg.Usage != nil {
