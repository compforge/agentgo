...
func runSummaryCompaction(
	...
	historyOpts := []agentgo.CallOption{
		agentgo.WithMaxTokens(int(float64(cfg.ReserveTokens) * 0.8)),
		agentgo.WithThinking(agentgo.ThinkingOff),
	}
	prefixOpts := []agentgo.CallOption{
		agentgo.WithMaxTokens(int(float64(cfg.ReserveTokens) * 0.5)),
		agentgo.WithThinking(agentgo.ThinkingOff),
	}
