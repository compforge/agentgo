...
//
// Implementations typically wrap agentgo.AgentLoop. Tests inject a stub.
...
type TurnExecutor func(ctx context.Context, msgs []agentgo.AgentMessage) ([]agentgo.AgentMessage, error)

// ProtocolHooks lets the application layer plug its own wire format and
// policy decisions into the runner without forking it. agentgo stays
...
type RunConfig struct {
	...
	// nil field falls back to the agentgo default (see ProtocolHooks).
...
func Run(ctx context.Context, cfg RunConfig) error {
	...
	history := append([]agentgo.AgentMessage(nil), cfg.History...)
	...
	for {
		...
		userMsg := agentgo.UserMsg(currentPrompt)
		turnInput := make([]agentgo.AgentMessage, 0, len(history)+1)
...
func lastAssistantText(msgs []agentgo.AgentMessage) string {
	for i := len(msgs) - 1; i >= 0; i-- {
		m := msgs[i]
		if m == nil || m.GetRole() != agentgo.RoleAssistant {
			continue
		}
