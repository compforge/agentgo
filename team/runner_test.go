...
// agentgo does not expose a public AssistantMsg constructor, so tests build
// the value directly.
func assistantMsg(text string) agentgo.AgentMessage {
...
type stubExecutor struct {
	mu       sync.Mutex
	calls    [][]agentgo.AgentMessage
	respond  func(idx int, msgs []agentgo.AgentMessage) ([]agentgo.AgentMessage, error)
	done     chan int
...
func (s *stubExecutor) Execute(ctx context.Context, msgs []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) {
	s.mu.Lock()
	idx := len(s.calls)
	clone := make([]agentgo.AgentMessage, len(msgs))
	copy(clone, msgs)
...
func (s *stubExecutor) Calls() [][]agentgo.AgentMessage {
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make([][]agentgo.AgentMessage, len(s.calls))
	copy(out, s.calls)
	return out
}
...
// value yields the agentgo defaults (passthrough format, no idle, no
...
// Default hooks pass the InitialPrompt through verbatim — agentgo makes no
...
func TestRun_HistoryAccumulatesAcrossTurns(t *testing.T) {
	...
	h.exec.respond = func(idx int, _ []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) {
		return []agentgo.AgentMessage{assistantMsg("ack " + string(rune('0'+idx)))}, nil
	}
	h.start(t, "kickoff")
	defer h.stop(t, time.Second)
	...
	if calls[1][0].GetRole() != agentgo.RoleUser {
		t.Errorf("history[0] role = %v, want user", calls[1][0].GetRole())
	}
	if calls[1][1].GetRole() != agentgo.RoleAssistant {
...
func TestRun_HistorySeedsFirstTurnPrefix(t *testing.T) {
	h := newHarness(t)
	seed := []agentgo.AgentMessage{
		agentgo.UserMsg("earlier question"),
	...
	if first[0].GetRole() != agentgo.RoleUser || first[0].TextContent() != "earlier question" {
		t.Errorf("history[0] = (%v, %q), want (user, 'earlier question')", first[0].GetRole(), first[0].TextContent())
	}
	if first[1].GetRole() != agentgo.RoleAssistant || first[1].TextContent() != "earlier answer" {
		t.Errorf("history[1] = (%v, %q), want (assistant, 'earlier answer')", first[1].GetRole(), first[1].TextContent())
	}
	if first[2].GetRole() != agentgo.RoleUser || first[2].TextContent() != "continue please" {
...
func TestRun_HistorySeedIsCopied(t *testing.T) {
	h := newHarness(t)
	h.exec.respond = func(idx int, _ []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) {
		return []agentgo.AgentMessage{assistantMsg("ack")}, nil
	}
	// Over-capacity slice so an un-copied append would write into seed[1:].
	seed := make([]agentgo.AgentMessage, 1, 8)
	seed[0] = agentgo.UserMsg("seed-msg")
	ctx, cancel := context.WithCancel(context.Background())
	h.cancel = cancel
	go func() {
		h.runErr <- Run(ctx, RunConfig{
...
func TestRun_IdleHookReceivesLastAssistantText(t *testing.T) {
	h := newHarness(t)
	h.exec.respond = func(idx int, _ []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) {
		return []agentgo.AgentMessage{
			agentgo.UserMsg("tool-result placeholder"),
...
func TestRun_ShouldTerminateHookExitsCleanly(t *testing.T) {
	h := newHarness(t)
	h.exec.respond = func(idx int, _ []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) {
		return []agentgo.AgentMessage{assistantMsg("ok")}, nil
	}
	th := makeTestHooks()
...
func TestRun_ValidatesRequiredFields(t *testing.T) {
	cases := []struct {
		...
		{"no identity", RunConfig{Registry: NewRegistry(), TaskRT: task.NewRuntime(), TaskID: "t", Execute: func(context.Context, []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) { return nil, nil }}},
		{"no registry", RunConfig{Identity: &task.Identity{AgentName: "x"}, TaskRT: task.NewRuntime(), TaskID: "t", Execute: func(context.Context, []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) { return nil, nil }}},
		{"no task runtime", RunConfig{Identity: &task.Identity{AgentName: "x"}, Registry: NewRegistry(), TaskID: "t", Execute: func(context.Context, []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) { return nil, nil }}},
		{"no task id", RunConfig{Identity: &task.Identity{AgentName: "x"}, Registry: NewRegistry(), TaskRT: task.NewRuntime(), Execute: func(context.Context, []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) { return nil, nil }}},
...
func TestSpawn_ForwardsProtocolHooks(t *testing.T) {
	...
	rt := task.NewRuntime()
	exec := newStubExecutor()
	exec.respond = func(int, []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) {
		return []agentgo.AgentMessage{assistantMsg("ack")}, nil
	}
...
func TestRun_IdleClaim_FeedsSyntheticPromptWithoutMailbox(t *testing.T) {
	h := newHarness(t)
	h.exec.respond = func(idx int, _ []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) {
		return []agentgo.AgentMessage{assistantMsg("done-" + strings.Repeat("x", idx+1))}, nil
	}
...
func TestRun_IdleClaim_MailboxBeatsClaimWhenBothReady(t *testing.T) {
	...
	h.exec.respond = func(int, []agentgo.AgentMessage) ([]agentgo.AgentMessage, error) {
		return []agentgo.AgentMessage{assistantMsg("ok")}, nil
	}
	h.startWithHooks(t, "kickoff", hooks)
	h.exec.waitForCalls(t, 2, time.Second)
...
func lastUserText(msgs []agentgo.AgentMessage) string {
	if len(msgs) == 0 {
		return ""
	}
	last := msgs[len(msgs)-1]
	if last == nil || last.GetRole() != agentgo.RoleUser {
		return ""
	}
