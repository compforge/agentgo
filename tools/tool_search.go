...
import (
	...
	"github.com/compforge/agentgo/permission"
)

// ToolSearchTool provides LLM-driven discovery of deferred tools.
// It implements agentgo.Tool, agentgo.ContentTool, and agentgo.DeferFilter.
//
...
func NewToolSearchTool(deferred ...agentgo.Tool) *ToolSearchTool {
...
// ExecuteContent implements agentgo.ContentTool.
// Returns tool_reference content blocks for matched tools.
func (t *ToolSearchTool) ExecuteContent(_ context.Context, args json.RawMessage) ([]agentgo.ContentBlock, error) {
	...
	if len(matches) == 0 {
		return []agentgo.ContentBlock{agentgo.TextBlock("No matching tools found.")}, nil
	}

	blocks := make([]agentgo.ContentBlock, 0, len(matches)+1)
	for _, name := range matches {
		blocks = append(blocks, agentgo.ToolRefBlock(name))
	}
	blocks = append(blocks, agentgo.TextBlock("Tool loaded."))
	return blocks, nil
}

// Execute implements agentgo.Tool. Delegates to ExecuteContent and returns
...
// IsDeferred implements agentgo.DeferFilter.
...
// WasDeferred implements agentgo.DeferFilter.
