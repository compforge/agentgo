...
type BashTool struct {
	...
	notifyFn        func(agentgo.AgentMessage)
...
func (t *BashTool) SetNotifyFn(fn func(agentgo.AgentMessage)) {
	t.notifyFn = fn
}
...
func (t *BashTool) executeForeground(ctx context.Context, a bashArgs) (json.RawMessage, error) {
		...
		for {
			...
			if n > 0 {
				...
				for {
					...
					agentgo.ReportToolProgress(ctx, agentgo.ProgressPayload{Kind: agentgo.ProgressSummary, Summary: string(append([]byte(nil), pending[:idx]...))})
			...
			if readE != nil {
				if !(errors.Is(readE, io.EOF) || errors.Is(readE, os.ErrClosed)) {
					...
					agentgo.ReportToolProgress(ctx, agentgo.ProgressPayload{Kind: agentgo.ProgressSummary, Summary: string(append([]byte(nil), pending...))})
				}
				return
			}
		}
	...
	if len(outStr) > defaultMaxBytes {
		if f, ferr := os.CreateTemp("", "agentgo-bash-*.log"); ferr == nil {
