...
func TestTransformMessages_ThinkingBlocks_Anthropic(t *testing.T) {
	msgs := []agentgo.Message{
		{
			...
			Content: []agentgo.ContentBlock{
				agentgo.ThinkingBlock("let me think..."),
				agentgo.TextBlock("hello"),
			},
	...
	if result[0].Content[0].Type != agentgo.ContentThinking {
...
func TestTransformMessages_ThinkingBlocks_NonAnthropic(t *testing.T) {
	msgs := []agentgo.Message{
		{
			Role: agentgo.RoleAssistant,
			Content: []agentgo.ContentBlock{
				agentgo.ThinkingBlock("reasoning here"),
				agentgo.TextBlock("answer"),
			},
			StopReason: agentgo.StopReasonStop,
		},
	}
	...
	if result[0].Content[0].Type != agentgo.ContentText {
...
func TestTransformMessages_SkipErroredMessages(t *testing.T) {
	msgs := []agentgo.Message{
		agentgo.UserMsg("hi"),
		{Role: agentgo.RoleAssistant, Content: []agentgo.ContentBlock{agentgo.TextBlock("oops")}, StopReason: agentgo.StopReasonError},
		{Role: agentgo.RoleAssistant, Content: []agentgo.ContentBlock{agentgo.TextBlock("ok")}, StopReason: agentgo.StopReasonStop},
	}
	...
	msgs := []agentgo.Message{
		{
			Role: agentgo.RoleAssistant,
			Content: []agentgo.ContentBlock{
				agentgo.ToolCallBlock(agentgo.ToolCall{ID: longID, Name: "test", Args: json.RawMessage(`{}`)}),
			},
			StopReason: agentgo.StopReasonToolUse,
		},
		agentgo.ToolResultMsg(longID, json.RawMessage(`"ok"`), false),
	}
	...
	msgs := []agentgo.Message{
		{
			Role: agentgo.RoleAssistant,
			Content: []agentgo.ContentBlock{
				agentgo.ToolCallBlock(agentgo.ToolCall{ID: badID, Name: "test", Args: json.RawMessage(`{}`)}),
			},
			StopReason: agentgo.StopReasonToolUse,
		},
		agentgo.ToolResultMsg(badID, json.RawMessage(`"result"`), false),
	}
...
func TestTransformMessages_OrphanedToolCall(t *testing.T) {
	msgs := []agentgo.Message{
		{
			Role: agentgo.RoleAssistant,
			Content: []agentgo.ContentBlock{
				agentgo.ToolCallBlock(agentgo.ToolCall{ID: "tc1", Name: "test", Args: json.RawMessage(`{}`)}),
			},
			StopReason: agentgo.StopReasonToolUse,
		},
	...
	if result[1].Role != agentgo.RoleTool {
...
func TestBuildToolCall_InvalidArgsMessageStaysJSONSerializable(t *testing.T) {
	...
	msg := agentgo.Message{
		Role:    agentgo.RoleAssistant,
		Content: []agentgo.ContentBlock{agentgo.ToolCallBlock(tc)},
	}
