// Package llm adapts LLM providers to the [agentgo.ChatModel] interface. It
// wraps litellm to reach OpenAI, Anthropic, Gemini, and other backends, and
// classifies provider errors onto agentgo's retry and overflow contracts.
...
// ProviderConfig is kept for source compatibility with older agentgo callers.
// New litellm providers expose explicit Config structs; this package maps the
// common subset that agentgo needs.
...
// Implements agentgo.ProviderNamer for per-provider API key resolution.
...
func (l *LiteLLMAdapter) GenerateStream(ctx context.Context, messages []agentgo.Message, tools []agentgo.ToolSpec, opts ...agentgo.CallOption) (<-chan agentgo.StreamEvent, error) {
	...
	eventChan := make(chan agentgo.StreamEvent, 100)
	...
	go func() {
		...
		resp, err := litellm.Handle(stream, func(ev litellm.Event) error {
			...
			case litellm.ReasoningDelta:
				...
				if thinkIdx < 0 {
					partial.Content = append(partial.Content, agentgo.ThinkingBlock(""))
					thinkIdx = len(partial.Content) - 1
					eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventThinkingStart, ContentIndex: thinkIdx, Message: partial}
				}
				partial.Content[thinkIdx].Thinking += e.Text
				eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventThinkingDelta, ContentIndex: thinkIdx, Delta: e.Text, Message: partial}
			case litellm.ContentDelta:
				if textIdx < 0 {
					partial.Content = append(partial.Content, agentgo.TextBlock(""))
					textIdx = len(partial.Content) - 1
					eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventTextStart, ContentIndex: textIdx, Message: partial}
				}
				partial.Content[textIdx].Text += e.Text
				eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventTextDelta, ContentIndex: textIdx, Delta: e.Text, Message: partial}
			...
			case litellm.ToolUseStart:
				...
				partial.Content = append(partial.Content, agentgo.ToolCallBlock(agentgo.ToolCall{ID: e.ID, Name: e.Name, ThoughtSignature: e.Signature}))
				idx := len(partial.Content) - 1
				if key != "" {
					toolBlockIndices[key] = idx
				}
				eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventToolCallStart, Message: partial}
			case litellm.ToolUseDelta:
				...
				if len(e.ArgumentsDelta) > 0 {
					...
					eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventToolCallDelta, Delta: string(e.ArgumentsDelta), Message: partial}
				}
			case litellm.ToolUseDone:
				key := toolUseEventKey(e.ID, e.ItemID, e.Index, e.OutputIndex)
				idx := findPendingToolCallBlock(partial.Content, toolBlockIndices, key)
				var current agentgo.ToolCall
				...
				if idx >= 0 {
					partial.Content[idx] = agentgo.ToolCallBlock(completed)
				} else {
					partial.Content = append(partial.Content, agentgo.ToolCallBlock(completed))
					idx = len(partial.Content) - 1
				}
				eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventToolCallEnd, ContentIndex: idx, Message: partial, CompletedToolCall: &completed}
			}
			return nil
		})

		if textIdx >= 0 {
			eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventTextEnd, ContentIndex: textIdx, Message: partial}
		}
		if thinkIdx >= 0 {
			eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventThinkingEnd, ContentIndex: thinkIdx, Message: partial}
		}

		if err != nil {
			eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventError, Err: wrapProviderError(err)}
			return
		}
		...
		eventChan <- agentgo.StreamEvent{Type: agentgo.StreamEventDone, Message: partial, StopReason: partial.StopReason}
	}()

	return eventChan, nil
}

func finalizePendingStreamToolCalls(content []agentgo.ContentBlock, indices map[string]int, argsByKey map[string][]byte) []agentgo.ContentBlock {
	if len(indices) == 0 {
		return content
	}
	for key, idx := range indices {
		...
		content[idx] = agentgo.ToolCallBlock(completed)
	}
	return content
}

// convertMessages converts agentgo.Message to litellm.Message.
...
func convertMessages(messages []agentgo.Message) []litellm.Message {
...
func isReasoningOnlyStopAssistant(orig agentgo.Message, converted litellm.Message) bool {
	if orig.StopReason != agentgo.StopReasonStop {
		return false
	}
...
func hasImageContent(blocks []agentgo.ContentBlock) bool {
	for _, b := range blocks {
		if b.Type == agentgo.ContentImage && b.Image != nil {
			return true
		}
	}
	return false
...
// convertSingleMessage converts one agentgo.Message to litellm.Message.
func convertSingleMessage(msg agentgo.Message) litellm.Message {
...
func convertAgentBlocks(content []agentgo.ContentBlock, cache *litellm.CacheControl) []litellm.Block {
...
func sanitizeOutgoingToolCall(tc agentgo.ToolCall) agentgo.ToolCall {
...
func convertImageBlock(img agentgo.ImageData) litellm.ImageBlock {
	if img.URL != "" {
...
// convertResponse converts litellm.Response to agentgo.Message with content blocks.
func convertResponse(response *litellm.Response) agentgo.Message {
	content := convertResponseContent(response)

	var usage *agentgo.Usage
...
func mergeResponseMetadata(message *agentgo.Message, response *litellm.Response) {
	if message == nil {
		return
	}
...
func convertResponseContent(response *litellm.Response) []agentgo.ContentBlock {
	if response == nil {
		return nil
	}
	var content []agentgo.ContentBlock
	for _, block := range response.Blocks {
		switch b := block.(type) {
		case litellm.TextBlock:
			content = append(content, agentgo.TextBlock(b.Text))
		case litellm.ReasoningBlock:
			if b.Text != "" {
				content = append(content, agentgo.ThinkingBlock(b.Text))
			}
		case litellm.ToolUseBlock:
			content = append(content, agentgo.ToolCallBlock(buildToolCall(b.ID, b.Name, string(b.Arguments), b.Signature)))
		}
	}
	return content
}
...
func pricingForUsage(fallback *ModelPricing, usage *agentgo.Usage) *ModelPricing {
	return fallback
}

// mapStopReason maps litellm canonical FinishReason to agentgo StopReason.
func mapStopReason(reason litellm.FinishReason) agentgo.StopReason {
	switch reason {
	...
	default:
		return agentgo.StopReason(string(reason))
	}
}
...
func applyCallConfig(ctx context.Context, req *litellm.Request, opts []agentgo.CallOption, caps litellm.Capabilities) (context.Context, error) {
	callCfg := agentgo.ResolveCallConfig(opts)
...
func convertResponseFormat(format *agentgo.ResponseFormat) (*litellm.ResponseFormat, error) {
	if format == nil {
		return nil, nil
	}
...
func applyToolConfig(request *litellm.Request, tools []agentgo.ToolSpec) error {
	if len(tools) == 0 {
		return nil
	}
...
// buildToolCall constructs an agentgo.ToolCall from raw litellm fields,
// routing malformed args into dedicated diagnostic fields (see ToolCall doc).
func buildToolCall(id, name, rawArgs, thoughtSignature string) agentgo.ToolCall {
	n := normalizeArgs(rawArgs)
	return agentgo.ToolCall{
		ID:               id,
		Name:             name,
		Args:             n.Args,
...
func findPendingToolCallBlock(content []agentgo.ContentBlock, byID map[string]int, toolCallID string) int {
	if toolCallID != "" {
